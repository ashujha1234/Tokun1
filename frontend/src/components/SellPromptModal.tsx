...
// function RecordingPill({ time }: { time: string }) {
//   const [dot, setDot] = useState(true);

//   useEffect(() => {
//     const id = window.setInterval(() => setDot((d) => !d), 800);
...
//         }}
//       />
//       <span style={{ color: "#111", fontWeight: 500 }}>Screen recording</span>
//       <span
//         style={{
//           marginLeft: 4,
//           background: GRAD,
...
//   const [oneTimeSale, setOneTimeSale] = useState(false);
//   const [title, setTitle] = useState("");
//   const [description, setDescription] = useState("");
//   const [promptText, setPromptText] = useState("");
//   const [category, setCategory] = useState("");
...
//       {controlledOpen === undefined && (
//         <DialogTrigger asChild>
//           <Button className="rounded-lg px-4 py-2 text-white" style={{ backgroundImage: GRAD }}>
//             Upload Your Prompt
//           </Button>
//         </DialogTrigger>
//       )}
...
//               Category <span className="text-pink-400">*</span>
//             </Label>
//             <Select value={category} onValueChange={setCategory} required disabled={catsLoading || !!catsError}>
//               <SelectTrigger className={fieldBase}>
//                 <SelectValue placeholder={catsLoading ? "Loading categories..." : "Select a category"} />
//               </SelectTrigger>
//               <SelectContent className="max-h-[240px]">
...
//             </Select>
//             {catsError && (
//               <p className="text-xs text-red-400">Couldn't load categories. Try reopening this modal to retry.</p>
//             )}
//           </div>
...
//             {isPaid && (
//               <div className="flex items-center justify-between text-sm">
//                 <Label htmlFor="one-time-sale" className="text-white">One-time sale</Label>
//                 <Switch
//                   id="one-time-sale"
//                   checked={oneTimeSale}
//                   onCheckedChange={setOneTimeSale}
...
//                       )}
//                       <div>
//                         <p className="max-w-48 truncate text-sm font-medium">{file.name}</p>
//                         <p className="text-xs text-white/60">{formatFileSize(file.size)}</p>
//                       </div>
//                     </div>
//                     <Button type="button" variant="ghost" size="sm" onClick={() => removeAttachment(index)} className="text-red-500 hover:text-red-600">
//                       <FileX className="h-4 w-4" />
//                     </Button>
//                   </div>
//                 ))}
...
export default function SellPromptModal({
  ...
  // ✅ FIX: valid categories only (empty name wali SelectItem crash karati hai)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let res: Response;
      try {
        res = await fetch(`${API_BASE}/api/prompt`, {
          method: "POST",
          headers: { ...(token ? { Authorization: `Bearer ${token}` } : {}) },
          body: fd,
          credentials: "include",
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="no-scrollbar flex-1 overflow-y-auto px-6 pb-6 space-y-6 pt-4">
          ...
          <div className="space-y-2">
            ...
            {/* ✅ FIX: error par Select disable nahi hota — sirf loading par */}
            <Select value={category} onValueChange={setCategory} required disabled={catsLoading}>
              <SelectTrigger className={fieldBase}>
                <SelectValue placeholder={catsLoading ? "Loading categories..." : "Select a category"} />
              </SelectTrigger>
          ...
          <div className="flex items-center justify-end gap-3 pt-2">
            ...
            <Button
              type="submit"
              disabled={uploading}
              className="rounded-xl px-4 py-2.5 text-sm font-medium text-white shadow-md disabled:opacity-60"
              style={{ backgroundImage: GRAD }}
            >
              {uploading
                ? "Uploading…"
