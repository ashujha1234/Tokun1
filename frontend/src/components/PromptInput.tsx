...
//       // 2) Build preview text then get REAL counts for both original + optimized text
// const option = generateOptimizedVersion(text, optimizedText);

// const originalCount = await llmService.countTokens(text);
// const optimizedCount = await llmService.countTokens(optimizedText);

// const originalTokens = Number(originalCount?.tokens || 0);
// const tokens = Number(optimizedCount?.tokens || 0);
// const words = Number(optimizedCount?.words || 0);
...
//       const saved = await saveOptimization(option.text, tokens, originalTokens);

//       if (saved?.data?.user?.lifetimeTokunSaved !== undefined) {
//   localStorage.setItem(
//     "tokun_lifetime_saved_tokens",
//     String(saved.data.user.lifetimeTokunSaved)
//   );
//   window.dispatchEvent(new Event("tokun:lifetime-saved-updated"));
// }
...
//     ? lastUsage.total
//     : Math.max(1, Number.isFinite(optimizedTokens) ? optimizedTokens : 1);
...
// const saved = await saveOptimization(textToSave, tokens, tokens);
//         id = saved.id || null;
