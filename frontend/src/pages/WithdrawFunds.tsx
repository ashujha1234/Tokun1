...
// //     withdrawAmount > 0 && !isOverBalance && !isBelowMinimum && !!selectedAccount;
...
// //                 {/* Withdrawal amount box: text ke thoda niche tak */}
// //                 <div
...
// //                     <div
// //                       className="mt-5 flex items-center justify-between px-8"
// //                       style={{
// //                         width: "min(546px, 100%)",
// //                         height: 60,
// //                         borderRadius: 16,
...
// //                       >
// //                         {isOverBalance
// //                           ? `You can withdraw up to ₹${availableToWithdraw.toLocaleString(
// //                               "en-IN"
// //                             )} only.`
...
// //                 {/* Payment method box alag rakha hai */}
// //                 <div
// //                   className="relative self-start overflow-hidden border border-white/10"
// //                   style={{
// //                     height: "fit-content",
...
// //   - Import useAuth
// //   - Fetch real wallet balance from GET /api/wallet/balance
...
//   useEffect(() => { fetchBankAccounts(); }, [WALLET_ACCOUNTS_KEY]);
...
//     const bankName = bankForm.bankName.trim();
//     setFormError("");
//     if (!holder || !accNum || !confirmAccNum || !ifsc || !bankName) { setFormError("Please fill out all fields."); return; }
//     if (accNum !== confirmAccNum) { setFormError("Account numbers do not match."); return; }
//     setSaveLoading(true);
//     const authToken = getAuthToken();
//     const headers: Record<string, string> = { "Content-Type": "application/json" };
//     if (authToken) headers.Authorization = `Bearer ${authToken}`;
//     const body = { accountHolderName: holder, accountNumber: accNum, confirmAccountNumber: confirmAccNum, ifscCode: ifsc, bankName, default: accounts.length === 0 };
//     try {
//       let newAccount: WalletAccount | null = null;
//       try {
//         const res = await fetch(BANK_ADD_URL, { method: "POST", headers, body: JSON.stringify(body), credentials: "include" });
//         const data = await res.json().catch(() => ({}));
//         if (!res.ok) {
//           const code = data?.error || `http_${res.status}`;
//           const message = code === "all_fields_required" ? "Please fill out all fields." : code === "account_numbers_mismatch" ? "Account numbers do not match." : code === "account_already_exists" ? "This bank account is already saved." : "Could not add bank account.";
//           throw new Error(message);
//         }
//         newAccount = mapApiAccount(data?.bankAccount);
//       } catch (apiErr: any) {
//         if (authToken) throw apiErr;
//         newAccount = { id: crypto.randomUUID(), name: bankName, last4: accNum.slice(-4), ifsc, isDefault: accounts.length === 0, iconBg: "bg-[#1A73E8]/25", iconColor: "text-[#1A73E8]" };
//       }
//       setAccounts((prev) => { const without = prev.filter((a) => !(a.name === newAccount!.name && a.last4 === newAccount!.last4)); return [...without, newAccount!]; });
//       setSelectedAccountId(newAccount.id);
//       setLatestAddedAccount(newAccount);
//       setAddBankOpen(false);
//       resetBankForm();
...
//       <main className="relative z-10 px-4 pt-[130px] pb-20">
//         <section className="mx-auto overflow-hidden" style={{ width: "min(1024px, 100%)", minHeight: 953, borderRadius: 30, background: "#21212180", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", fontFamily: fontBase }}>
//           <div className="p-8 sm:p-[50px]">
...
//                 Securely transfer your earnings to your preferred account.<br />Withdrawals are typically processed within 24 hours.
//               </p>
//             </div>
...
//                   <div className="absolute inset-0 bg-[radial-gradient(circle_at_55%_90%,rgba(255,20,239,0.24),transparent_45%)]" />
//                   <div className="relative z-10 p-8">
//                     <p style={{ fontFamily: fontBase, fontWeight: 600, fontSize: 12, letterSpacing: "1.2px", color: "#C084FC", textTransform: "uppercase" }}>Available to Withdraw</p>
...
//                       style={{ width: "min(546px, 100%)", height: 60, borderRadius: 16, background: "#18181B80", border: isOverBalance ? "1px solid rgba(248,113,113,0.75)" : "1px solid #FFFFFF1A" }}>
//                       <div className="flex min-w-0 flex-1 items-center gap-4">
//                         <span style={{ fontFamily: fontBase, fontWeight: 900, fontSize: 34, color: "#C084FC" }}>₹</span>
//                         <input value={amount} onChange={(e) => handleAmountChange(e.target.value)} placeholder="0.00" inputMode="numeric"
//                           className="min-w-0 flex-1 bg-transparent outline-none placeholder:text-white/10"
//                           style={{ fontFamily: fontBase, fontWeight: 900, fontSize: 34, color: "#FFFFFF" }} />
//                       </div>
//                       <button type="button" onClick={() => setAmount(String(availableToWithdraw))}
...
//                     <div className="mt-4 flex items-center gap-2">
//                       <Info className="h-4 w-4 text-[#71717A]" />
//                       <span style={{ fontFamily: fontBase, fontWeight: 400, fontSize: 12, color: "#71717A" }}>Minimum withdrawal: ₹100. Standard transaction fees may apply.</span>
//                     </div>
//                     {(isOverBalance || isBelowMinimum) && (
//                       <p className="mt-3" style={{ fontFamily: fontBase, fontWeight: 500, fontSize: 12, lineHeight: "16px", color: "#F87171" }}>
//                         {isOverBalance ? `You can withdraw up to ₹${fmt(availableToWithdraw)} only.` : "Minimum withdrawal amount is ₹100."}
//                       </p>
//                     )}
//                   </div>
//                 </div>
...
//                             </div>
//                             <p className="mt-5 truncate px-3" style={{ fontFamily: fontBase, fontWeight: 700, fontSize: 14, color: "#FFFFFF", maxWidth: "100%" }}>{account.name}</p>
//                             <p className="mt-3" style={{ fontFamily: fontBase, fontWeight: 400, fontSize: 10, color: "rgba(255,255,255,0.35)" }}>•••• {account.last4}</p>
//                           </button>
//                         );
//                       })}
//                       <button type="button" onClick={() => { resetBankForm(); setAddBankOpen(true); }}
//                         className="flex h-[125px] flex-col items-center justify-center rounded-[10px] border border-dashed"
//                         style={{ background: "rgba(255,255,255,0.05)", borderColor: "rgba(255,255,255,0.2)" }}>
//                         <div className="grid h-10 w-10 place-items-center rounded-full bg-white/10">
//                           <span aria-hidden style={plusMaskStyle(14, "#A1A1AA")} />
//                         </div>
//                         <p className="mt-5" style={{ fontFamily: fontBase, fontWeight: 700, fontSize: 14, color: "#FFFFFF" }}>Add Card</p>
//                       </button>
//                     </div>
//                   </div>
//                 </div>
...
//                     <div className="h-px w-full bg-white/10" />
//                     <div className="flex items-center justify-between gap-3 min-w-0">
//                       <span style={{ fontFamily: fontBase, fontWeight: 700, fontSize: 20, color: "#FFFFFF", whiteSpace: "nowrap", flexShrink: 0 }}>Estimated Total</span>
...
//             {formError && (
//               <div className="mt-5 rounded-xl border px-4 py-3 text-sm text-white" style={{ background: "#2A1717", borderColor: "rgba(239,68,68,0.35)" }}>{formError}</div>
//             )}
//             <div className="mt-6 space-y-5">
//               <div>
//                 <label className="mb-2 block text-sm text-white/80">Account holder name</label>
//                 <input value={bankForm.holder} onChange={(e) => setBankForm((p) => ({ ...p, holder: onlyLetters(e.target.value) }))} className="w-full rounded-md border border-white/15 bg-[#17171A] px-4 py-3 text-white placeholder-white/35 outline-none" placeholder="Enter account holder name" />
//               </div>
//               <div>
//                 <label className="mb-2 block text-sm text-white/80">Account number</label>
//                 <input value={bankForm.accNum} onChange={(e) => setBankForm((p) => ({ ...p, accNum: onlyDigits(e.target.value) }))} inputMode="numeric" className="w-full rounded-md border border-white/15 bg-[#17171A] px-4 py-3 text-white placeholder-white/35 outline-none" placeholder="Enter account number" />
//               </div>
//               <div>
//                 <label className="mb-2 block text-sm text-white/80">Confirm account number</label>
//                 <input value={bankForm.confirmAccNum} onChange={(e) => setBankForm((p) => ({ ...p, confirmAccNum: onlyDigits(e.target.value) }))} inputMode="numeric" className="w-full rounded-md border border-white/15 bg-[#17171A] px-4 py-3 text-white placeholder-white/35 outline-none" placeholder="Re-enter account number" />
//               </div>
//               <div>
//                 <label className="mb-2 block text-sm text-white/80">IFSC Code</label>
//                 <div className="relative">
//                   <input value={bankForm.ifsc} onChange={(e) => setBankForm((p) => ({ ...p, ifsc: e.target.value.toUpperCase() }))} className="w-full rounded-md border border-white/15 bg-[#17171A] px-4 py-3 pr-[112px] text-white placeholder-white/35 outline-none" placeholder="IFSC Code" />
//                   <button type="button" className="absolute bottom-1 right-1 top-1 rounded-md px-4 text-sm text-white" style={{ background: "linear-gradient(270deg,#FF14EF 0%,#1A73E8 100%)" }}>Find IFSC</button>
//                 </div>
//               </div>
//               <div>
//                 <label className="mb-2 block text-sm text-white/80">Bank name</label>
//                 <input value={bankForm.bankName} onChange={(e) => setBankForm((p) => ({ ...p, bankName: e.target.value }))} className="w-full rounded-md border border-white/15 bg-[#17171A] px-4 py-3 text-white placeholder-white/35 outline-none" placeholder="Bank name" />
//               </div>
//               <div className="flex items-center justify-end gap-3 pt-2">
...
//                 <button type="button" onClick={handleSaveBank} disabled={saveLoading} className="h-[49px] w-[162px] rounded-[6px] px-[15px] text-white disabled:opacity-60" style={{ background: "linear-gradient(270deg,#FF14EF 0%,#1A73E8 100%)" }}>
//                   {saveLoading ? "Saving..." : "Save & Continue"}
//                 </button>
//               </div>
//             </div>
...
//             <button type="button" aria-label="Close" onClick={() => setSuccessPopupOpen(false)} className="absolute right-5 top-5 text-white/50 hover:text-white"><X className="h-5 w-5" /></button>
//             <div className="mx-auto grid h-[80px] w-[80px] place-items-center rounded-full bg-[#052A1D]">
//               <div className="grid h-[36px] w-[36px] place-items-center rounded-full bg-[#21B37A]">
//                 <Check className="h-5 w-5 text-black" strokeWidth={3} />
//               </div>
...
//             </div>
//             <div className="mt-8 flex items-center justify-center gap-6">
//               <button type="button" onClick={() => { setSuccessPopupOpen(false); resetBankForm(); setAddBankOpen(true); }} style={{ fontFamily: fontBase, fontWeight: 400, fontSize: 16, color: "#FFFFFF" }}>Add Another</button>
//               <button type="button" onClick={() => { setSuccessPopupOpen(false); setAddBankOpen(false); }} className="h-[50px] rounded-[7px] px-5" style={{ background: "#333335", fontFamily: fontBase, fontWeight: 400, fontSize: 16, color: "#FFFFFF" }}>View</button>
//             </div>
//           </div>
//         </div>
//       )}
