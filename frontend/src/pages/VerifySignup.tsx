...
/* The box is emptied, because whatever is in it is now guaranteed wrong.
   signup/initiate overwrites the pending account's otpHash, so the code typed
   from the first email stops working the moment the second one is sent — but it
   stayed sitting in the field, four digits that look exactly like a filled-in
   answer, with Verify enabled and ready to fail. Cleared only on success: if the
   resend was refused (rate limit, network) the earlier code is still the live one
   and throwing it away would be destroying a valid answer. */
setOtp("");
focusOtpField();
