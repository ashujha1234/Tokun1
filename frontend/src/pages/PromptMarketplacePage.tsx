...
/** How many category chips the search results row will show at once, across
    both of its groups. A display cap, not a data one — the grid itself is
    never limited by it. */
const FACET_CAP = 14;
...
const LibSearchResults = ({
  ...
  loading: boolean;
  facetLabel: string;
  facets: { name: string; count: number; isChild?: boolean }[];
  ...
  /* Two headings, not one. `facetLabel` names the searched category's own
     sub-categories; anything else in the row matched on text rather than
     taxonomy (a Travel product whose description says "design"), so filing it
     under "<Category> sub-categories" stated something false about the
     taxonomy. Split out, both groups stay honest and nothing is dropped.

     When there is no searched category to have children of — a free-text query
     like "cyberpunk" — every chip is an "other" and one group under the
     fallback label is the whole row, exactly as before. */
  const childFacets = facets.filter((f) => f.isChild);
  const otherFacets = facets.filter((f) => !f.isChild);
  const facetGroups = childFacets.length
    ? [
        { key: "child", label: facetLabel, items: childFacets },
        { key: "other", label: "Other categories", items: otherFacets },
      ].filter((group) => group.items.length > 0)
    : [{ key: "all", label: facetLabel, items: facets }];
            ...
            {facetGroups.map((group, groupIndex) => (
              <div key={group.key} className={groupIndex > 0 ? "mt-4" : ""}>
                <div className="flex items-center gap-2 mb-3">
                  {/* The icon marks the filter row as a whole, so only the
                      first heading carries it — repeating it would read as two
                      unrelated controls rather than one row in two parts. */}
                  {groupIndex === 0 ? (
                ...
                <div className="flex flex-wrap gap-2">
                  {/* "All" clears the whole row, not just this group, so it
                      belongs to the first heading only. */}
                  {groupIndex === 0 && (
                    <button
                      type="button"
...
// removed along with it.
  ...
  /* Razorpay is fetched when a buy is actually started, not on mount — see
     src/lib/razorpayCheckout.ts. `rzpReady` is kept only so the button's
     "still loading" state still exists for the window between the click and
     the script arriving; it starts false and the guard below awaits. */
...
const filteredPrompts = prompts.filter((p) => {
  ...
  /* The sub-category chips: the searched category's own children ("design" →
     ...
     Each chip is tagged `isChild` so the row can say which is which. Without
     that tag every chip sat under one "Design sub-categories" heading, so
     "Travel" and "Solo Travel" — carried in by a travel product whose text
     happens to mention design — were presented as sub-categories of Design,
     which they are not. They still belong in the row (they are a real part of
     the result set), just not under that heading; the render splits them into
     their own group.

     Children sort first so the cap below can never drop the searched
     category's own split in favour of incidental matches. Either way only
     names with at least one result survive — a chip that empties the grid is a
     dead end, not a filter. */
  const searchFacets = useMemo(() => {
    if (!searchTerm) return [] as { name: string; count: number; isChild: boolean }[];
    ...
    const childKeys = new Set(
      searchSubOptions.map((name) => name.trim().toLowerCase()).filter(Boolean)
    );

    /* Keyed by lowercase name so the taxonomy's "Logo & Branding" and a
       result's "logo & branding" can't both become chips. */
    const merged = new Map<string, { name: string; count: number; isChild: boolean }>();
    const add = (name: string, count: number) => {
      const key = name.trim().toLowerCase();
      if (!key || merged.has(key)) return;
      merged.set(key, { name, count, isChild: childKeys.has(key) });
    };
    ...
    /* The cap is split between the groups rather than applied to one ranked
       list. Children get first call on it — they are the meaningful split of
       what was searched — but never all of it: a search whose category has 14+
       populated children would otherwise push every "other" chip out, and
       results filed outside the searched tree would go unnamed again, which is
       the thing this row exists to prevent. */
    const children = ranked.filter((f) => f.isChild);
    const others = ranked.filter((f) => !f.isChild);
    const otherSlots = Math.min(others.length, Math.max(FACET_CAP - children.length, 4));

    return [...children.slice(0, FACET_CAP - otherSlots), ...others.slice(0, otherSlots)];
 ...
 const handlePurchase = async (prompt: Prompt) => {
  ...
  if (!token) {
    ...
    /* Fetched here rather than on page load: most visitors to the
       marketplace never buy, and this is ~100 kB of third-party script. The
       await is the old "still initializing" wait, moved to the one moment it
       matters. */
    if (!rzpReady) {
 ...
 const startPayment = async (prompt: Prompt) => {
    ...
    try {
      ...
      const data = await res.json();
      /* The two KYC_REQUIRED branches that were here are gone. No route ever
         answered with that code — the middlewares that produced it were mounted
         on nothing — so this was dead in both copies, and the second was
         unreachable after the first anyway. */
...
/* ensureKycVerified lived here and was never called by anything — which was
   just as well: it fetched `http://localhost:5002/api/kyc/status`, a hardcoded
   dev host, and on the failure that guarantees in production it opened the KYC
   modal rather than letting the purchase through. */
