...
const AddFunds = () => {
  ...
  const validateUpiIfNeeded = async () => {
    ...
    // UPI ID optional hai.
    // Blank rahe to Razorpay Checkout me QR/app/UPI option choose kar sakte ho.
    if (!cleanUpi) return true;
  ...
  const getRazorpayConfig = () => {
    ...
    return {
      display: {
        ...
        sequence: ["block.netbanking_block"],
        preferences: {
          show_default_blocks: false,
        },
      },
...
const handleConfirmAddFunds = async () => {
  ...
  try {
    ...
    const options = {
      ...
      handler: async function (response: any) {
        try {
          ...
          console.error("[AddFunds] verify failed:", err);
          setPaymentError(err?.message || "Payment verification failed.");
        } finally {
          setPayLoading(false);
        }
    ...
    razorpay.on("payment.failed", function (response: any) {
      console.error("[AddFunds] Razorpay payment failed:", response);
    ...
    console.error("[AddFunds] payment start error:", err);
    setPaymentError(err?.message || "Could not start payment.");
    setPayLoading(false);
  }
};
