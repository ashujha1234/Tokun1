// Razorpay's `profile.category` / `profile.subcategory` enum, transcribed from
// the Business Category / Business Sub-Category tables in
// https://razorpay.com/docs/payments/route/integration-guide/
//
// These strings go through to Razorpay's API verbatim, so nothing here may be
// re-worded — the human-readable labels are derived in the UI instead
// (underscores to spaces), which keeps this file a pure mirror of the docs.
//
// The pair is validated together, not field-by-field: a subcategory only means
// anything inside its own category, and Razorpay rejects a cross-category pair
// (which is what "ecommerce"/"marketplace" was — `marketplace` lives under no
// category at all; the ecommerce one is spelled `ecommerce_marketplace`).
//
// The docs also list an `others` category, deliberately omitted here: it is the
// one category with no subcategories, and Razorpay wants both halves. Nothing
// to offer the seller, so it isn't offered.
...
// True only for a category+subcategory pair that exists together in the table
// above. Both halves are checked in one call precisely because neither is
// meaningful alone.
function isValidCategoryPair(category, subcategory) {
  const subcategories = BUSINESS_CATEGORIES[category];
  return Array.isArray(subcategories) && subcategories.includes(subcategory);
}

module.exports = { BUSINESS_CATEGORIES, isValidCategoryPair };
